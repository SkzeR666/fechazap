...
export async function POST(request: Request) {
  try {
    ...
    const { data, error } = await auth.db.from('services').insert({ user_id: auth.user.id, name: body.name, description: body.description, price_cents: body.priceCents, active: body.active, sort_order: body.sortOrder }).select().single();
